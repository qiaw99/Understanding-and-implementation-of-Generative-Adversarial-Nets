...
def discriminator_loss(logits_real, logits_fake):
    ...
    dx = bce_loss(logits_real, true_labels)
    dgx = bce_loss(logits_fake, fake_labels)
    
    loss = torch.mean(dx) + torch.mean(dgx)
...
def generator_loss(logits_fake):
    ...
    fake_labels = torch.ones(logits_fake.shape[0]).type(dtype)
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    loss = torch.mean(0.5 * torch.square(scores_real - 1) + 0.5 * torch.square(scores_fake))
